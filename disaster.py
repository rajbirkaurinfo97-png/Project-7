...
# ==========================================
# 2. LOCAL MODEL LOADING (RELATIVE PATH)
# ==========================================
@st.cache_resource(show_spinner="Loading Fine-Tuned MiniLM Model Weights...")
def load_local_model():
    # Dynamic relative path pointing to the model directory inside your repository
    base_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(base_dir, "disaster_model")
...
try:
    ...
    st.error(f"⚠️ Error loading model from directory. Details: {e}")
    st.stop()
